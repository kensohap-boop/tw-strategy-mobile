...
def process_stock(code,name,market,rows,mktmap,start_date):
    ...
    for i,r in enumerate(rows):
        ...
        d0s.append({"code":code,"name":name,"date":r["date"],"close":round(r["close"],4),"combo_id":cid})
        ...
        for sid,ei,ep in entries:
            ...
            if o and o["strategy_return_pct"] is not None:
                samples.append({"scenario":sid,"combo_id":cid,"code":code,"name":name,"d0_date":r["date"],"entry_date":rows[ei]["date"],"entry_price":ep,**o})
    return samples,d0s
...
def main():
    ...
    result={'schema_version':'2026-09-02.backtest10y.1','generated_at':now.isoformat(),'period':{'start':start,'end':now.date().isoformat(),'years':a.years},'methodology':{'purpose':'exploratory historical baseline; forward daily data remains out-of-sample validation','universe':'current TWSE/TPEx universe; survivorship bias possible','price_source':'Yahoo Finance daily chart, OHLC adjusted by adjclose/close factor','limit_up_note':'historical limit-up close approximated as adjusted close return >=9.5%; validate important rows against official data','mother_filter':'MA5/10/20 spread <=3%; D0 return >0; D0 close above MA5/10/20','exit_rules':['intraday high reaches +20%: sell half of current remaining at +20%','close below MA5: sell half of current remaining','close below MA10: sell all remaining','D10: sell all remaining','same-day priority MA10 > +20% > MA5']},'universe_count':len(items),'failed_symbols':failures,'d0_count':len(all_d0),'completed_strategy_samples':len(all_samples),'yearly':[{'year':y,'samples':len(v),'avg_return_pct':round(sum(v)/len(v),4),'profit_rate_pct':round(sum(x>0 for x in v)/len(v)*100,2)} for y,v in sorted(yearly.items())],'combination_stats':rows,'top20':ranked[:20]}
